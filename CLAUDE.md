...
`PROGRESS.md` is a single cross-cutting project checklist, edited from whichever branch the work happened on. **It is the only place completion is tracked.** `spec/VaultSigner-Spec.md`'s own `## 12. Execution plan` checklist is a static plan — its `- [ ]` items are never checked off, even for items that are fully done and shipped (compare macOS's Phase 2 items there against `PROGRESS.md`'s own Phase 2 section, which marks the same items `- [x]` with the verification details). When an item is done, add or update its `- [x] N.M ...` entry in `PROGRESS.md` — don't edit the checkbox in the spec file itself.

**Keep `PROGRESS.md` entries to a checklist line plus, where needed, a short exception note justifying a deviation — not a narrative.** Detailed debugging history, dead ends, and "here's exactly what happened" material belongs in a dev journal instead: each platform keeps its own at `apps/<platform>/docs/<platform>-dev-journal.md` (e.g. `apps/android/docs/android-dev-journal.md`), committed on that platform's own branch like everything else under `apps/<platform>/`; cross-platform-relevant facts cherry-picked from that work (an architectural decision or gotcha worth another platform knowing about ahead of time) go in `docs/shared-dev-journal.md` on `shared`. Reference the relevant journal from `PROGRESS.md`'s exception note rather than inlining the detail there.
