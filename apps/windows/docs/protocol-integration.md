---
title: Windows Protocol Integration
---
...
1. Try to connect. If it fails because the pipe doesn't exist, `VaultSignerAgent` isn't running — tell the user to launch `VaultSignerAgent.exe`. Unlike macOS (where the main app embeds and launches the agent as a login item), Windows currently ships them as two independently-launched processes — `VaultSignerUI.exe` does **not** start the agent for you yet. A third-party app has no way to launch the agent itself either way; the correct behavior is prompting the user, not trying to work around it.
2. `vaultsigner.list_public_keys` returning an empty list is not an error — it means no compartment is currently unlocked. Discovering a key requires the user to have unlocked its compartment in `VaultSignerUI.exe` first, or have auto-unlock configured for it via Settings → "Auto-unlock this compartment on startup" (DPAPI-backed — see `PROGRESS.md`'s Phase 3 notes for its disclosed limitation vs. macOS Keychain); your app has no way to trigger either path itself. Prompt the user to open `VaultSignerUI.exe` and unlock their vault, then retry.
