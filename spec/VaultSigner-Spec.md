...
- The service owns: FIDO2 listener registration, the custom-protocol listener (Section 7, where applicable), the in-memory key cache/retention timer (4.5), and is the sole writer of the container file (4.6). The management UI is a separate process the service can launch for password prompts, and which the user can independently open for management tasks; both communicate over the same local-IPC mechanism as Section 7, using an internal-only method namespace. The UI process requests container mutations from the service rather than writing the file itself.
